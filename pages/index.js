    ...
    if (type === "column") {
      ...
      if (idOfCol === col && idTask !== -1) {
        if (idOfTask !== idTask && oldCol !== col) {
          ...
          const newPlace = _columnTask.tasksIds.findIndex(
            item => item.id === idTask
          )
          ...
          this.setState({
            columns: columns.map(item =>
              item.id === idOfCol ? newTaskList : item
            ),
          })
        }
        ...
        if (!isTask) {
          ...
          const oldTask = oldColumn.tasksIds.findIndex(
            item => item.id === idOfTask
          )
        ...
        const column = this.state.columns.find(item => item.id === col)
        const isTaskList = column.tasksIds.length

        if (isTaskList === 0) {
          let oldColumn = columns.find(item => item.id === idOfCol)
          const _task = oldColumn.tasksIds.find(task => task.id === idOfTask)
          const task = oldColumn.tasksIds.findIndex(
            item => item.id === idOfTask
          )
